#include "SART_cuda3.h" // consists all required package and functions
...
for (int ibin = 0; ibin < n_bin; ibin++){
    ...
    for (int iter = 0; iter < n_iter; iter++){ // iteration
        ...
        for (int i_view = n_views[ibin]; i_view < n_views[ibin + 1]; i_view++){ // view
            ...
            kernel_projection(d_singleViewProj2, d_singleViewImg1, angle, SO, SD, da, na, ai, db, nb, bi, nx, ny, nz); // TBD
            ...
            kernel_projection(d_singleViewProj2, d_singleViewImg1, angle, SO, SD, da, na, ai, db, nb, bi, nx, ny, nz);
