...
kernel_projection<<<gridSize, blockSize>>>(d_proj, d_img, angle, SO, SD, da, na, ai, db, nb, bi, nx, ny, nz);
cudaMemcpy(h_proj, d_proj, na * nb * sizeof(float), cudaMemcpyDeviceToHost);
cudaFree(d_proj);
cudaFree(d_img);
